...
# Load model
diet_model = pickle.load(open('diet_recommendation_model.pkl', 'rb'))
...
def predict_diet_with_bmi(data, bmi, systolic_bp, diastolic_bp, smoking, diabetes, heart_attack):
    ...
    print("BMI:", bmi)
    print("BP:", systolic_bp, "/", diastolic_bp)
    print("Smoking:", smoking, "Diabetes:", diabetes, "Heart attack:", heart_attack)
    ...
    try:
        df = pd.DataFrame([[data["Age"], data["Gender"], data["Total Cholesterol"],
                            ...
                            bmi, systolic_bp, diastolic_bp, smoking, diabetes, heart_attack]],
                          columns=["age", "sex", "total_cholesterol", "ldl", "hdl", 
                                   "non_hdl", "bmi", "systolic_bp", "diastolic_bp", 
                                   "smoking", "diabetes", "heart_attack"])
        print("DataFrame for prediction:\n", df)

        label = diet_model.predict(df)[0]
...
def get_meal_plan(name, age, gender, data, bmi,food):
    ...
    try:
        ...
        if response and hasattr(response, 'text'):
            ...
            print("Error Occrued while feaching Data")
...
def upload_file():
    if request.method == 'POST':
        ...
        meal_plan = get_meal_plan(name, age, gender, extracted_data, bmi,food)
